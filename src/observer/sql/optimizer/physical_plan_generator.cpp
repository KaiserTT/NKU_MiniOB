...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_log_oper, std::unique_ptr<PhysicalOperator> &oper)
{
  std::vector<std::unique_ptr<LogicalOperator>> &child_opers = update_log_oper.children();
