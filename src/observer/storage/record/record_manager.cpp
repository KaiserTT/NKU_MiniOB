...
RC RecordPageHandler::update_record(RID *rid, int offset, int len, Value &value) {
  ASSERT(readonly_ == false, "cannot update record in page while the page is readonly");
  ...
  // 获取原数据
  char* src_data = frame_->data() + page_header_->first_record_offset + (rid->slot_num * page_header_->record_size);
